 ...
 * 
 * This class doesn't actually store the contents of this container - that is all being modeled by Elko.
 * So this.contents(INDEX) finds the item in Elko contents.
 * 
 * Note that there aren't many verb related methods here, as GET/PUT semantics are managed by
 * verbs on the object being GET/PUT into or out-of the container. 
 */
public abstract class Container extends HabitatMod {
	 ...
	 * Now it's an get function with the same name for easy porting. The contents are managed by Elko.
	 * 
	...
	public void get_container_contents(User from) {
		...
		item.sendObjectDescription(context(), context());			//* TODO CONNECTION_JSON only - sends extra container MAKE.
		// Assumes calling code has already accounted for any client-side storage limitation
	}
	...
	public void close_container(User from) {
		/* Original code was in regionproc.pl1 - ELKO handles this now */
		// TODO Client Memory Management and several messages are missing from this interim implementation        
