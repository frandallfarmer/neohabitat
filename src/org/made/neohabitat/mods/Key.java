 ...
 * The only way to use it is for the Avatar to be holding (slot:HANDS)
 * and pointing at the object and using the appropriate verb.
 * Any lock/unlock operation will be a side effect on the pointed 
 * object. For example, a Bag's OPENCONTAINER operation will 
 * test any Key while attempting to open it.
 * 
 * @author randy
 *
 */
...
public class Key extends HabitatMod {
	...
	public int		HabitatClass 	 () { return CLASS_KEY; }
	public String	HabitatModName	 () { return "Key"; }
	public int		capacity 		 () { return 0; }
	public int		pc_state_bytes 	 () { return 2; };
	public boolean	known 			 () { return true; }
	public boolean	opaque_container () { return false; }
	public boolean	filler 			 () { return false; }

	/** Least significant byte in a 16 bit value to match against a lock in order to lock/unlock the item.*/
	public int 		key_number_lo	= 0;
	/** Most significant byte in a 16 bit value to match against a lock in order to lock/unlock the item.*/
	public int		key_number_hi	= 0;	

	@JSONMethod({ "style", "x", "y", "orientation", "gr_state", "key_number_lo", "key_number_hi" })  
	public Key(OptInteger style, OptInteger x, OptInteger y,
	...
	public JSONLiteral encode(EncodeControl control) {
		...
		if (0 != key_number_lo)				{ result.addParameter("key_number_lo",		key_number_lo); }
		if (0 != key_number_hi)				{ result.addParameter("key_number_hi",		key_number_hi); }	
		result.finish();
		return result;
	}
