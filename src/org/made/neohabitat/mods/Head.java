 ...
 * 
 * The Head is a complex object. When it's sitting in the region
 * or a container, it acts like any other object.
 * 
 * But, when carried (HANDS slot) or worn (HEAD slot) it is
 * generally treated as part of the containing avatar's body.
 * 
 * @author randy
 *
 */
...
public class Head extends HabitatMod {
	...
	public int		HabitatClass 	 () { return CLASS_HEAD; }
	public String	HabitatModName	 () { return "Head"; }
	...
	public Head(OptInteger style, OptInteger x, OptInteger y,
	 ...
	 */
	@JSONMethod
	public void HELP (User from)	{ head_HELP(from); }
	 ...
	 */
	@JSONMethod
	public void GET (User from)		{ head_WEAR(from); }
	 ...
	 */	
	@JSONMethod
	public void WEAR (User from)	{ head_WEAR(from); }
	 ...
	 */	
	@JSONMethod
	public void REMOVE (User from)	{ head_REMOVE(from); }
	...
	public void head_WEAR(User from) {
		...
		if (holding(avatar, this)) {				// First handle putting the head "on" from HANDS to HEAD slot.
			...
			/* inc_record(avatarptr, HS$body_changes); */	// TODO Hall of Records
			send_neighbor_msg(from, avatar.noid, "WEAR$");
			send_reply_success(from);
		} else if (cont.HabitatClass() != CLASS_AVATAR || cont.noid == avatar.noid) { 
			generic_GET(from);	  // Otherwise, get it from pocket or another container the normal way.
		} else {
			send_reply_error(from);  // Should only happen when trying to grab a head out of someones else's pocket.
		}
	}
	...
	public void head_HELP(User from) {
		...
		if (cont.noid != THE_REGION) {
			if (cont.HabitatClass() == CLASS_AVATAR) {
				avatar((User) cont.object()).avatar_IDENTIFY(from, this.noid); /* Warning!  Tricky stuff here! */
				return;
			}
		}
		send_reply_msg(from, "HEAD: point at your body and select PUT to wear head.  GETing the head will remove it if you are wearing it.");
	}
}
