...
public class Avatar extends Container implements UserMod {
	...
	public int		HabitatClass 	 () { return CLASS_AVATAR; }
	public String	HabitatModName	 () { return "Avatar"; }
	public int		capacity 		 () { return AVATAR_CAPACITY; }
	public int		pc_state_bytes 	 () { return 6; };
	public boolean	known 			 () { return true; }
	public boolean	opaque_container () { return true; }
	...
	protected static int ConnectionType = CONNECTION_JSON;						/* Soon to default to CONNECTION_HABITAT */
	/**
	 * Set the ConnectionType for this user.
	 * @param type CONNECTION_JSON or CONNECTION_HABITAT
	 */
	public static void setConnectionType (int type)	{ ConnectionType = type; }
	/**
	 * Get the ConnectionType for this user.
	 * @return CONNECTION_JSON or CONNECTION_HABITAT
	 */
	public static int  getConnectionType ()			{ return ConnectionType; }
	...
	/** Target NOID and magic item saved between events, such as for the GOD TOOL (see Magical.java). This is a transient value and not persisted. */
	public HabitatMod	savedTarget		= null;
	public Magical		savedMagical	= null;
	
	@JSONMethod({ "style", "x", "y", "orientation", "gr_state", "nitty_bits", "bodyType", "stun_count",
		"bankBalance", "activity", "action", "health", "restrainer", "custom"})  
	public Avatar( OptInteger style, OptInteger x, OptInteger y, OptInteger orientation, 
			OptInteger gr_state, OptInteger nitty_bits, OptString bodyType, OptInteger stun_count, 
			OptInteger bankBalance, OptInteger activity, OptInteger action, OptInteger health,
			OptInteger restrainer, int[] custom) {
	 ...
	 */
	@JSONMethod
	public void GRAB (User from) {unsupported_reply(from, noid, "Avatar.GRAB not implemented yet.");}
	 ...
	 */	
	@JSONMethod
	public void HAND (User from) {unsupported_reply(from, noid, "Avatar.HAND not implemented yet.");}
...
//     if (selfptr == avatarptr)  {							TODO Bullet Proofing Needed
 ...
 //    }
	 ...
	 * 
	 * @param from User representing the connection making the request.
	 * @param esp Byte flag indicating that ESP message mode is active on the client.
...
*/  
		...
		send_neighbor_msg(from, this.noid, "WALK$",
							"x",	destination_x,
							"y",	destination_y,
							"how",	walk_how);
		send_reply_msg(from, this.noid,
						"x",	destination_x,
						"y",	destination_y,
						"how",	walk_how);
	 ...
	 */	
	@JSONMethod
	public void DISCORPORATE (User from) {unsupported_reply(from, noid, "Avatar.DISCORPORATE not implemented yet.");}
	 ...
	 */	
	@JSONMethod
	public void ESP (User from) {unsupported_reply(from, noid, "Avatar.ESP not implemented yet.");}
	 ...
	 */	
	@JSONMethod
	public void SIT (User from) {unsupported_reply(from, noid, "Avatar.SIT not implemented yet.");}
	 ...
	 */	
	@JSONMethod
	public void TOUCH (User from) {unsupported_reply(from, noid, "Avatar.TOUCH not implemented yet.");}
	...
	public void FNKEY (User from, OptInteger key, OptInteger target) {
		...
		case 16: // F8
			...
			object_say(from, "                                 v.01 ");
			object_say(from, "    The MADE, The Museum of Arts and Digital Entertainment, Oakland CA");
			object_say(from, "                                      ");
	 ...
	 * 
	 * @param from User representing the connection making the request.
	 * @param replyNoid which avatar.noid is the one getting the reply.
	 */
...
//			p_msg_s(from, avatar.noid, targetUser, SPEAK$, "I am " + avatarname);
			/* OBSOLETE CODE REFACTORED
			 ...
			 * 
			 * if (UserList(self.avatarslot)->u.online)
		...
		send_reply_msg(from, replyNoid, "text", result);		/* TODO is replyNoid right? */
	}
	...
	public void avatar_NEWREGION(User from, int direction, int passage_id) {
		...
		if (direction >= 1 && direction <= 4) {
		   int direction_index = (direction + region.orientation + 2) % 4;
			String new_region = region.neighbors[direction_index]; // East, West, North, South
