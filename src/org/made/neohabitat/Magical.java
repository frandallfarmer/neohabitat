...
public abstract class Magical extends HabitatMod {
	...
	public JSONLiteral encodeMagical(JSONLiteral result) {
		...
		if (0 != magic_type)	{ result.addParameter("magic_type", magic_type); }
		if (0 != charges) 		{ result.addParameter("charges", charges); }
		if (0 != magic_data)	{ result.addParameter("magic_data",  magic_data); }
		if (0 != magic_data2)	{ result.addParameter("magic_data2", magic_data2); }
		if (0 != magic_data3)	{ result.addParameter("magic_data3", magic_data3); }
		if (0 != magic_data4)	{ result.addParameter("magic_data4", magic_data4); }
		if (0 != magic_data5)	{ result.addParameter("magic_data5", magic_data4); }
		return result;
	}
	...
	private void god_tool(User from, HabitatMod target) {
		...
		avatar.savedTarget	= target; // Save the target for future GOD TOOL commands. Does not persist.
		avatar.savedMagical	= this;	  // We have to return to this object to keep running GOD commands.
	...
	private void modify_variable(User from, HabitatMod target, int offset, int new_value) {
		...
		send_broadcast_msg(THE_REGION, "FIDDLE_$", "target", target.noid, "offset", offset, "argCount", 1, "value", new_value);
	}

	
	/**
	 * CALLBACK for the GOD TOOL - it must reconstruct the context for interpretation from an earlier request.
	 * @param from	The user holding the magical GOD TOOL object, and pointing at avatar.saveTargetNoid.
	 */
	public void god_tool_revisited(User from, String request_string) {
		...
		if (len > 0) {
			...
			case 'p':			// Set pattern (arg 0-14)
				target.orientation = (target.orientation & FACING_BIT) |
									 (arg << 3 & PATTERN_BITS)		   &
									 BYTE_MASK;
				modify_variable(from, target, C64_ORIENT_OFFSET, target.orientation);
				break;
			...
			case 't':			// Set text field value
				...
				break;
			case 'n':		// enter god mode on a noid (useful for unclickable objects)
			...
			case 'h':
				...
				object_say(from, "s#-gr.state, " +
						    	 (char) ARROW_U + (char) ARROW_D + (char) ARROW_L + (char) ARROW_R +
						                          "#-move, tSetSignText");
				break;
			}
			return;
	...
	private static final String magic_help[] = {
