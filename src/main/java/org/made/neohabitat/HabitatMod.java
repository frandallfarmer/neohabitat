 ...
 * 
 * All of the game-specific behavior is either in this class or in one of it's descendants.
 * It implements defaults for all the required verbs, the common state that every habitat object has,
 * and lots of generic behavior.
 * 
 * The Elko Open Source project provides a replacement for all of the critical services provided by
 * Q-Link/AOL. It also provides all of the connection management, object database, and message dispatching
 * services that were part of Habitat's server core (non-game logic).
 * 
 * ALL of the original source that is ported here was written in a 1980's flavor of Stratus PL1.
 * PL1 was all about globals, pass by reference, and no support for classes.
 * 30 years ago, Chip Morningstar wrote the bulk of the Habitat game-logic code, simulating structures,
 * classes, and a form of class inheritance by concatenating include files and careful
 * management of procedure references.
 * 
 * The game-logic port of PL1 Habitat to Elko Habitat [Java] focuses on clarity of checking that
 * the translated code is correct and visibly similar to the original code.
 * 
 * This means that some of the coding practices evolved over the last three decades have been
 * set aside. For example - naming conventions about mixed-caps and underscores have been "rolled back"
 * to 1988 - at least in terms of the behavior code. Any 100% new Java code may use modern naming
 * conventions are used (For example, the Habitat mod names are all start with a capital letter.)
 * 
 * The non-static constant globals present a different problem and have been replaced with public state
 * that is accessed by getter functions. So the PL1 contents array has been replaced with contents().
 * Likewise bit-flags are now boolean arrays in memory and they are [un]packed for storage/transmission as needed.
 * And simple dereference functions such as avatar(), container(), current_region(), and position() are provided
 * where globals once were.
 * 
 * @author randy
 *
 */
public abstract class HabitatMod extends Mod implements HabitatVerbs, ObjectCompletionWatcher {
	...
	// public int avatarslot = 0;	Obsolete, the connection is clearly represented by the User
	// public int obj_id = 0;		Replaced with item.ref() getter
	/** 0-255. Ephemeral Numeric Object ID: The client's world model of region contents, 0 = THE_REGION. */
	public int noid = 0;
	...
	// public int position = 0;		position always == this.y, so replaced with a getter.
	public int gr_state = 0;
	// public int container = 0;	containership is managed by Elko - use it's understanding to access the objects
	 ...
	 * 
	 * @param user Who you want to get the Avatar Mod for.
	 ...
	 * 
	 * @param obj The habitat mod that wants to find it's container.
	...
	public JSONLiteral encodeCommon(JSONLiteral result) {
		...
		/* Do not do result.finsh() here. Each Habitat Class does the final assembly. */
		return result;
	}
	 ...
	 * 
	 * @param from User representing the connection making the request.
	 * @param request The string describing the rejected request.
	 */
	 ...
	 */
	@JSONMethod
	public void TEST (User from) { trace_msg("This.getClass().getName(): " + this.getClass().getName()); }

	/**
	 * Verb (Generic): Get HELP for this.
	 * 
	 * Unlike most verbs, HELP has a useful default implementation that applies to
	 * all classes that don't choose to override it.
	 * 
	 * @param from User representing the connection making the request.
	 */
	@JSONMethod
	public void HELP (User from) { generic_HELP(from); }
	 ...
	 */
	@JSONMethod
	public void ASK (User from) { illegal(from, this.HabitatModName() + ".ASK"); }
	 ...
	 */
	@JSONMethod
	public void DO (User from) { illegal(from, this.HabitatModName() + ".DO"); }
	 ...
	 */
	@JSONMethod
	public void RDO (User from) { illegal(from, this.HabitatModName() + ".RDO"); }
	 ...
	 */
	@JSONMethod
	public void GET (User from) { illegal(from, this.HabitatModName() + ".GET"); }
	...
	public void generic_GET (User from)		{
		...
		if ( !empty_handed(avatar(from)) ||
				!getable(this) || 
				( !accessable(this) && selfClass != CLASS_GAME_PIECE ) ||
				contClass == CLASS_BUREAUCRAT   ||
				contClass == CLASS_VENDO_FRONT  ||
				contClass == CLASS_VENDO_INSIDE ||
				contClass == CLASS_GLUE) {
			send_reply_error(from);
			return;
		}

		if ( (selfClass == CLASS_BOX || selfClass == CLASS_BAG) &&
				((Openable) this).open_flags[OPEN_BIT]) {
			send_reply_error(from);
			return;
		}
		if ((position() != HANDS || !grabable(this)) &&
				(contClass == CLASS_AVATAR && cont.noid != avatar(from).noid) ) {
			send_reply_error(from);
			return;
		}

		/* OBSOLETE CODE:
		 * An Elko User == Connection, so there is no Turned to Stone state.
		 * And since a Elko Habitat Avatar is attached 1:1 to a Elko User,
		 * this code can never be true in this server.
		 * 
		 * if (contClass == CLASS_AVATAR &&
		 * 	   ^UserList(cont.avatarslot)->u.online) { 
		 *		send_reply_error(from);
		 *		return;
		 * }
		 */
		...
		/* All the preemptive tests have passed, we can really try to pick this item up! */
		...
		/* int original_position = position() + 1; Original dead code that is never referenced. FRF */
		...
		/* If getting a switched on flashlight from an opaque container, turn up the lights. */
		if (selfClass == CLASS_FLASHLIGHT) {
			if (((Flashlight) this).on == TRUE) {
				if (container_is_opaque(cont, y)) {		/* TODO should y be position() here? FRF */
		...
		/* 	if (Avatar.getConnectionType() == CONNECTION_JSON && container_is_opaque(cont, y)) {
	 ...
	 * Drops the item at the avatar's feet.
	 * 
	 * @param from User representing the connection making the request.
	 */
	...
	/**
	 * Most attempt to PUT an item go through this code. There are lots of special cases.
	 * 
...
//		boolean put_success = false;				/* TODO FIX THIS ORIGINAL GLOBAL */
		/* Check to see of the object being PUT (this) is a container and it's open */

		if (this instanceof Openable && ((Openable) this).open_flags[OPEN_BIT]) {
			trace_msg ("PUT WHILE OPEN: " + from.ref() +
					" attempted to put " + item.ref() +
					" into containter " + cont.object().ref());
			send_reply_error(from);
			return;
		}
		...
		if (cont.noid != THE_REGION) {
			if (contClass != CLASS_AVATAR	&&
					cont instanceof Openable		&&
					!((Openable) cont).open_flags[OPEN_BIT] ) {
			...
			if (contClass == CLASS_AVATAR) j = j - 3;
			for (int i = 0; i <= j; i++) {
...
//				put_success = true;
		...
		/* TODO CLASS_PAPER	    
	    if (selfClass == CLASS_PAPER)
	         going_away_flag = ((Writable) this).text_id == NULL;
		 */
		...
		if (contClass == CLASS_DISPLAY_CASE && !going_away_flag) {
			/* TODO DisplayCase
	    	DisplayCase case = (DisplayCase) this;
...
//		put_success = true;
		...
		if (Avatar.getConnectionType() == CONNECTION_JSON) {
			if (container_is_opaque(oldContainer, oldY) && 
					!container_is_opaque(cont, y)) {
	 ...
	 * 
	 * @param from 		User representing the connection making the request.
	 * @param target	The new container
	 * @param target_x	The new horizontal position for this.
	 * @param target_y	The new vertical position for this.
	 */
...
//		boolean throw_success 			= false;				/* TODO FIX THIS ORIGINAL GLOBAL */
		...
		/* DEAD CODE
			if (target == null) {
				r_msg_4(from, target.noid, this.x, this.y, FALSE);
				return;
			}
		 */
		...
		if ( targetClass != CLASS_STREET && targetClass != CLASS_GROUND ) {
			if ( targetClass != CLASS_FLAT && 
					targetClass != CLASS_TRAPEZOID &&
					targetClass != CLASS_SUPER_TRAPEZOID ) {
				send_throw_reply(from, noid, target.noid, this.x, this.y, FALSE);
				return;
			}
		...
		/* call check_path(target_id, target_x, target_y, new_x, new_y, dummy);*/  // ORIGINAL NEVER IMPLEMENTED

		/* This check says, simply, "did it go where it was aimed?" */
		if (new_x != target_x | new_y != target_y) {				// This is dead code TODO Review to Remove
			send_throw_reply(from, noid, target.noid, this.x, this.y, FALSE);
			return;
		}
		/* Preflight complete! Lets throw the item into the region at the supplied coordinates */
...
//		throw_success = true;    
	 ...
	 * this generic implementation provides that. If no override is specified, this is
	 * is the HELP message handler.
	 * 
	 * @param from User representing the connection making the request.
	 */
	public void generic_HELP (User from) {
		final String help_messages[] = {
				"i",                                     /*   0 -- region */
				"i",                                     /*   1 -- avatar */
				"i",                                     /*   2 -- amulet */
				"-",                                     /*   3 */
		...
		if (the_message == "-") { /* non-existent objects */
			...
			trace_msg("Impossible help request, class " + this.getClass().getSimpleName() + ". Missing HELP implementation?");
	...
	/**
	 * @param packedBits The bits unpacked into a boolean array.
	 ...
	 * 
	 * @param item The thing being priced.
	 ...
	 * You see, the avatar is both transparent AND opaque.
	 * 
	 * @param cont	The container being tested
	 ...
	 * @return		Is this slot, of this container, visible to the region?
	 */
	// TODO @depracate Is this ever used properly? It would be bad to call this if CLASS_AVATAR
	 ...
	 * 
	 * @param who	The avatar being tested.
	 ...
	 * 
	 * @param avatar	The avatar being tested.
	...
	public boolean getable(HabitatMod object) {
		if (object.HabitatClass() == CLASS_ROCK ||
				object.HabitatClass() == CLASS_FLAG) { /* TODO OLD BUG? The struct_flag.pl1 does not have 'mass', but 'state' */
	...
	public boolean grabable(HabitatMod object) {
		...
		if ( current_region().nitty_bits[STEAL_FREE] ||
				object.HabitatClass() == CLASS_PAPER  ||
				object.HabitatClass() == CLASS_BOOK   ||
				object.HabitatClass() == CLASS_TOKENS ||
	 ...
	 * @param x			The horizontal position (not considered. Should be deprecated)
	 * @param y			The slot/vertical position
	 * @return			Is it OK to put the item at this position?
	 */
	public boolean available(HabitatMod container, int x, int y) {
		if (container == null) 					return false;
		if (container.noid == THE_REGION)		return true;
	...
	public boolean accessable(HabitatMod object) {
		if (container(object).noid == THE_REGION)
			...
			/* return(accessable(ObjList(object.container))); TODO Placeholder Recursive container walk */
			return true;
	}
	 ...
	 *      
	 * @param object	The object being tested.
	 * @param user		The User-avatar
	 * @return			Is our avatar standing in the right place to manipulate this object?
	 */
	...
	public boolean here(HabitatMod object, User user) {
		return(container(object).noid == THE_REGION &&
				object.x == avatar(user).x &&
				object.y == avatar(user).y);		
	}
	 ...
	 * @param new_container The target container
	 * @param new_position	The new position (slot)
	 * @param cp			Checkpoint flag
	 * @return				success
	 */
	...
	public boolean change_containers(HabitatMod obj, Container new_container, int new_position, boolean cp) {
		...
		/* Every check has passed. Proceed with container change (which should not fail.) */     
	 ...
	 * @param new_container	The new container.
	 * @return				Will the container change work on the client, considering limited memory?
	 */

	boolean heap_space_available(HabitatMod obj, Container new_container) {
		return true;
	}
	/* TODO This is a placeholder implementation. Client Storage is NOT currently managed FRF */
	 ...
	 * 
	 * @param obj		The object that is logging the message (e.g. Oracle Fountain.)
	 * @param avatar	The avatar that took the action to trigger the message.
	...
	public void message_to_god(HabitatMod obj, HabitatMod avatar, String msg) {
		...
		/* TODO Placeholder implementation by FRF - full implementation when CLASS_ORACLE is ported. */
	}

	/**
	 * An object sends a string message to a specific user
	 * 
	 * @param to User the message is going to.
	 * @param noid The object speaking to the user.
	 * @param text What the object wants to say.
	 */
	 ...
	 * 
	 * @param noid The object speaking to the region.
	 * @param text What the object wants to say.
	 */
	 ...
	 * Call this to say "not ready yet!" and reply with an error code.
	 * Hopefully the client will accept the result and proceed.
	 * 
	 * @param from The connection for this user.
	 * @param noid The noid that sent the request that is being unceremoniously terminated.
	 * @param text This error message text will be sent to the client of the user that issued the unsupported command.
	 */
	public void unsupported_reply(User from, int noid, String text) {
		object_say(from, text);
		send_reply_error(from, noid); // TODO Remove This last ditch attempt to keep the client running after a unsupported command arrives.
	}

	/**
	 * Create a JSONLiteral initialized with the minimum arguments for broadcast from the Habitat/Elko server.
	 * @param noid The object that is broadcasting.
	 * @param op The STRING name of the ASYNCRONOUS request. In the PL/1 source this was a numeric constant, not a string. i.e PL/1: SPEAK$ Elko: "SPEAK$". The lookup now occurs in the Client/Server bridge.
	...
	/**
	 * Create a JSONLiteral initialized with the minimum arguments needed for the Habitat/Elko server. Assumes this.noid is the object of interest.
	 * @param op The STRING name of the ASYNCRONOUS request. In the PL/1 source this was a numeric constant, not a string. i.e PL/1: SPEAK$ Elko: "SPEAK$". The lookup now occurs in the Client/Server bridge.
	 * @return message ready to add more parameters, finish(), and send.
	 */
	 ...
	 * 
	 * @param noid The object that is broadcasting.
	 * @param op The STRING name of the ASYNCRONOUS request. (See::new_broadcast_msg)
	 */
	 ...
	 * @param noid The object that is broadcasting.
	 * @param op The STRING name of the ASYNCRONOUS request. (See::new_broadcast_msg)
	 * @param text A string to send, will have attribute name "text"
	 */
	 ...
	 * @param noid The object that is broadcasting.
	 * @param op The STRING name of the ASYNCRONOUS request. (See::new_broadcast_msg)
	 * @param a1 First attribute to add
	 * @param v1 First value to add
	 * @param a2 Second attribute to add
	 * @param v2 Second value to add
	 */
	public void send_broadcast_msg(
			int noid, String op, 
			String a1, int v1,
			String a2, int v2) {
		JSONLiteral msg = new_broadcast_msg(noid, op);
	 ...
	 * @param a2 Second attribute to add
	 * @param v2 Second value to add
	 * @param a3 Third attribute to add
	 * @param v3 Third value to add
	 */
	public void send_broadcast_msg(
			int noid, String op, 
			String a1, int v1,
	 ...
	 * @param a3 Third attribute to add
	 * @param v3 Third value to add
	 * @param a4 Fourth attribute to add
	 * @param v4 Fourth value to add
	 */	
	public void send_broadcast_msg(
			int noid, String op, 
			String a1, int v1,
	 ...
	 * @param from The User/connection that gets the reply.
	 * @param text The string to send, added with the attribute name "text"
	 */
	 ...
	 * @param v3 Third value to add
	 */
	public void send_reply_msg(User from, int noid, String a1, int v1,
			String a2, int v2,
			String a3, int v3) {
		JSONLiteral msg = new_reply_msg(noid);
		msg.addParameter(a1, v1);
	...
	/**
	 * Create a JSONLiteral initialized with the minimum arguments to send to a user/connections "neighbors/other users" via the Habitat/Elko server. this.noid is used for the acting object.
	 * @param op The STRING name of the ASYNCRONOUS request. In the PL/1 source this was a numeric constant, not a string. i.e PL/1: SPEAK$ Elko: "SPEAK$". The lookup now occurs in the Client/Server bridge.
	 * @return message ready to add more parameters, finish(), and send.
	 */
	 ...
	 * 
	 * @param from The user/connection that is acting, and the only one that will NOT get the message.
	 * @param noid The object that is broadcasting.
	 * @param op The STRING name of the ASYNCRONOUS request. (See::new_neighbor_msg)
	 */	
	...
	/**
	 * Sends a ASYNCHRONOUS message to all the neighbors (other user/connections) in a region.
	 * this.noid is the acting object.
	 * 
	 * @param from The user/connection that is acting, and the only one that will NOT get the message.
	 * @param op The STRING name of the ASYNCRONOUS request. (See::new_neighbor_msg)
	 */	
	 ...
	 * @param noid The object that is broadcasting.
	 * @param op The STRING name of the ASYNCRONOUS request. (See::new_neighbor_msg)
	 * @param attrib First attribute to add
	...
	public void send_neighbor_msg(
			User from, int noid, String op, 
			String a1, int v1,
			String a2, int v2 ) {
		JSONLiteral msg = new_neighbor_msg(noid, op);
		msg.addParameter(a1, v1);
	...
	/**
	 * Create a JSONLiteral initialized with the minimum arguments to send a private message to a single targeted user/connection.
	 * this.noid is used as the acting object.
	 * 
	 * @param op The STRING name of the ASYNCRONOUS request. In the PL/1 source this was a numeric constant, not a string. i.e PL/1: SPEAK$ Elko: "SPEAK$". The lookup now occurs in the Client/Server bridge.
	 * @return message ready to add more parameters, finish(), and send.
	 */
	 ...
	 * @param to The user/connection that is the recipient of this private message.
	 * @param op The STRING name of the ASYNCRONOUS request. (See::new_private_msg)
	 * @param text A string to send. Will be added to the message as the "text" parameter.
	 */
	...
	public void send_private_msg(
			User from, int noid, User to, String op,
			String a1, int v1,
			String a2, int v2,
			String a3, int v3,
			String a4, int v4) {
	 ...
	 * @param bitpos	1-based bit position (right to left) to clear
	 ...
	 * 
	 * @param val		starting value
	 * @param bitpos	1-based bit position (right to left) to set
	 ...
	 * @param bitpos	1-based bit position (right to left) to test
