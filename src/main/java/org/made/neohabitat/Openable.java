 ...
 * 
 * There are generic support functions here for handling opening and closing a container as
 * well as locking and unlocking it (which requires a matching KEY.)
 */
public abstract class Openable extends Container {
	...
	/** Least significant byte in a 16 bit value to match against a key in order to lock/unlock the item.*/
	protected int 		key_lo			= 0;
	
	/** Most significant byte in a 16 bit value to match against a key in order to lock/unlock the item.*/
	...
	public JSONLiteral encodeOpenable(JSONLiteral result) {
		...
		if (0 != packBits(open_flags))	{ result.addParameter("open_flags",	packBits(open_flags)); }
		if (0 != key_lo)				{ result.addParameter("key_lo",		key_lo); }
		if (0 != key_hi)				{ result.addParameter("key_hi",		key_hi); }
		return result;
	}
	 ...
	 */
	@JSONMethod
	public void CLOSECONTAINER (User from)	{ generic_CLOSECONTAINER(from); }
	 ...
	 */
	@JSONMethod
	public void OPENCONTAINER (User from)	{ generic_OPENCONTAINER(from); }
	
	
	/**
	 * A generic HELP verb for containers that can open/close/lock/unlock.
	 * @param from			User representing the connection making the request.
	 * @param item_name		Each class will pass it's name (i.e. "Box") to customize this message.
	 * @param key_number	Combined value of key_lo and key_hi
	 * @param open_flags	The containers open/closed/locked/unlocked state.
	 */
	...
	public void generic_OPENCONTAINER (User from) {
		...
		boolean have_key = (held != null) &&
				(held.HabitatClass() == CLASS_KEY) &&
				(((Key) held).key_number_hi == key_hi) &&
				(((Key) held).key_number_lo == key_lo);
		if (!open_flags[OPEN_BIT] && 						// OPEN
				(have_key || open_flags[UNLOCKED_BIT]) &&   // Holding Key OR UNLOCKED
				container().noid == THE_REGION) {			// IN THE REGION (and nothing else.)
			open_flags[OPEN_BIT]	= true;					// TODO Not sure why setting the following state wasn't here in the original.
			open_flags[UNLOCKED_BIT]= true;
			gr_state 				= 1;
			gen_flags[MODIFIED]		= true;
			checkpoint_object(this);
			send_reply_success(from);			// TODO This reply wasn't here in the original. Why?
