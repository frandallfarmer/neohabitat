...
// The renderer-agnostic client shell. `adapter` supplies the region renderer + the coordinate-bound
// hooks (RegionView, Cursor, pickRegionTarget, edgeCoord, sideChevronStyle, onRegionReset,
// installFit); everything else here is shared UI/wiring and is the source of truth for both the 2D
// and 3D clients. Returns the App component. See lib/render2d-adapter.js / render3d-adapter.js.
export async function createClientShell(adapter) {
  if (SOUND_TRACE) {
  ...
  const keyboardTargetPick = () => {
    ...
    const pick = adapter.pickRegionTarget(pickState, at.canvasX, at.canvasY, at.scale)
  ...
  const resetForRegion = () => {
    ...
    adapter.onRegionReset?.()                            // clear_cache: drop decoded region art (2D trap cache)
  ...
  const connect = async (ws, context, user) => {
    ...
    globalThis.habitatVerbAt = (verb, canvasX, canvasY, args, scale = 3) => {
      const pick = adapter.pickRegionTarget(pickState, canvasX, canvasY, scale)
      if (!pick) return Promise.resolve({ ok: false, reason: "no-target" })
      if (pick.noid == null) return Promise.resolve({ ok: false, reason: "no-noid" })
      return dispatchVerb({ world, dispatch, dispatchClient, verb, noid: pick.noid, args: args ?? {}, pick })
    }
  ...
  const runRegionVerb = async (verb, { canvasX, canvasY, scale }, label) => {
    ...
    try {
      // (was dispatchVerbAtPick) — pick via the adapter, then the agnostic dispatchVerb. Same guards.
      const pick = adapter.pickRegionTarget(pickState, canvasX, canvasY, scale)
      if (!pick) return { ok: false, reason: "no-target" }
      if (pick.noid == null) return { ok: false, reason: "no-noid" }
      const result = await dispatchVerb({ world, dispatch, dispatchClient, verb, noid: pick.noid, args: {}, pick })
  ...
  const onRegionCommand = ({ command, cursorState, label, canvasX, canvasY, scale, habitatX }) => {
    ...
    const tpick = adapter.pickRegionTarget(pickState, canvasX, canvasY, scale)
  ...
  // Walk-off-edge: the renderer maps the edge (+ the click event) to an in-region GO coordinate
  // (2D: a point clamped to the walkable band, from the chevron's geometry); we then reproduce the
  // C64 region_change — GO/walk to that edge coordinate, THEN transit via the agnostic changeRegion.
  ...
  const App = ({ avatarName }) => {
    ...
    const sideChevStyle = adapter.sideChevronStyle?.(region) ?? ""
    balloonState.value.revision
...
// Boot: show the C64 title sequence (title.js) immediately — it loads instantly and supplies the
// click gesture audio needs — while the renderer adapter (heavy: region decoders / Three) and the
// shared shell load in the background. The title's "press any key" is gated on the load finishing.
// `makeAdapter` is the renderer's factory (sync or async); the 2D and 3D entry files each pass
// their own. Exported so both lib/live.js (2D) and lib/live3d.js (3D) are ~thin entries.
export function boot(makeAdapter) {
  ...
  Promise.resolve(makeAdapter())
    .then((adapter) => { adapter.installFit?.(root); return createClientShell(adapter) })
    .then((App) => { loadedApp.value = App })
