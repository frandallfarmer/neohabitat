...
type ElkoMessage struct {
	...
	// Custom: the five Hatchery appearance bytes a JSON web client sends with op
	// "CUSTOMIZE" (head_style, hair_pattern, av_orient, custom0, custom1) — the
	// JSON analogue of the binary CUSTOMIZE args. See handleJsonHatcheryCustomize.
	Custom           *[]uint8 `json:"custom,omitempty"`
	...
	// Hatchery: a web client sets hatchery:true on entercontext to advertise that
	// it can run the customizer (so a brand-new user gets the hatchery instead of a
	// default avatar). Bots never send it. See ensureUserCreated's capability gate.
	Hatchery       *bool          `json:"hatchery,omitempty"`
